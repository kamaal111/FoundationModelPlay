...
struct ItineraryHeader: View {
    ...
    var body: some View {
        ZStack(alignment: .topLeading) {
            ...
            Image("\(destination.backgroundImageName)-thumb")
                ...
                .mask {
                    Rectangle()
                        .fill(
                            Gradient(stops: [.init(color: .clear, location: 0.5), .init(color: .white, location: 0.6)])
        ...
        .mask {
            Rectangle()
                .fill(
                    Gradient(stops: [.init(color: .white, location: 0.3), .init(color: .clear, location: 1.0)])
                        .colorSpace(.perceptual)
                )
        }
        .ignoresSafeArea()
...
#Preview {
    ItineraryHeader(destination: Landmarks.first)
}
